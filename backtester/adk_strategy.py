...
class ADKStrategy(bt.Strategy):
    ...
    def _should_close_position(self) -> bool:
        ...
        if self.position.size > 0:  # Long position
            ...
            should_close = (
                price < self.trailing_stop or 
                self.rsi[0] > self.p.rsi_overbought
            )
            self.log(f"Long position - Price: {price}, Stop: {self.trailing_stop}, "
            ...
            self.log(f"Short position - Price: {price}, Stop: {self.trailing_stop}, "
                    f"RSI: {self.rsi[0]}, Close: {should_close}")
            return should_close
    ...
    def next(self):
        ...
        self.log(f"Processing bar {self.check_count} - Close: {self.data.close[0]}, "
