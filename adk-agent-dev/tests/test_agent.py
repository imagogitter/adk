...
def mock_market_tool():
    ...
    market_tool.get_market_info.return_value = {"limits": {"amount": {"min": 0.001, "max": 100.0}}}
...
def agent(mock_db_tool, mock_market_tool):
    ...
    with patch("agent.DatabaseTool") as mock_db, patch("agent.CCXTInfoTool") as mock_market:
...
def test_execute_trades(agent):
    ...
    agent.analyze_market = MagicMock(return_value={"signal": "buy", "reason": "bullish_trend"})
